...
import MainSafetyHubScreen from '@/components/Safety_Hub_App/Main_SafetyHub_Screen';
import {
  initialSafetyHubAppState,
  SafetyHubAppContext,
  SafetyHubAppContextProps,
  SafetyHubReducer,
} from '../sh_context';
import { SHSettingsScreen } from '@/components/Safety_Hub_App/components/05_Settings/SafetyHub_Settings';
import HazardsIncidentsScreen from '@/components/Safety_Hub_App/components/02_HazardsIncidents/02_HazardsIncidents';
import SafetyMeetingsScreen from '@/components/Safety_Hub_App/components/03_SafetyMeetings/SafetyMeetingsScreen';
import SafetyHubRecentActivity from '@/components/Safety_Hub_App/safetyhub_recent_activity';
import DiscussionTopicsScreen from '@/components/Safety_Hub_App/components/04_DiscussionTopics/DiscussionTopics';
import LiveBoardScreen from '@/components/Safety_Hub_App/components/01_LiveBoard/LiveBoardScreen';
import CreateSHHazards from '@/components/Safety_Hub_App/components/01_LiveBoard/SH_Hazard/Create_SH_Hazard';
...
function Page({ params }: any) {
  ...
  const contextSafetyHubManagerValue: SafetyHubAppContextProps = {
    state,
    dispatch,
  };
  ...
  useEffect(() => {
    if (appId) {
      dispatch({
        type: SAFETYHUBTYPE.ASSIGN_APP_ID,
        appId: appId,
      });
    }
  }, [appId]);
  ...
  useEffect(() => {
    ...
    const openSection =
      open && validSections.includes(open as (typeof validSections)[number])
        ? (open as (typeof validSections)[number])
        : null;
    if (openSection && appId) {
      dispatch({
        type: SAFETYHUBTYPE.SHOWPAGES,
        showPages: openSection,
      });
      router.replace(`/user/apps/sh/${appId}`, { scroll: false });
    }
  }, [appId, searchParams, dispatch, router]);

  return (
    <>
      <SafetyHubAppContext.Provider value={contextSafetyHubManagerValue}>
        <div
          className={`absolute inset-0 z-10 bg-white ${
            isNavbarVisible
              ? 'top-16 h-[calc(var(--app-vh)-70px)]'
              : 'h-[calc(var(--app-vh))]'
          } mx-auto w-full max-w-[1360px] overflow-y-hidden p-4`}
        >
          <div className="breadCrumbs flex justify-between border-b-2 border-[#eeeeee] pb-[15px]">
            <div className="flex justify-start gap-5">
              <div className="relative h-[50px] w-[50px] rounded-lg">
                <img src="/svg/sh/logo.svg" alt="show logo" />
              </div>
          ...
          <div className="flex w-full flex-col md:flex md:flex-row">
            ...
            />
            <div className="max-[664px] h-[calc(var(--app-vh)-70px)] w-full overflow-auto scrollbar-hide">
              {(state.showPages == undefined || state.showPages) && (
                <SafetyHubRecentActivity />
              )}
            </div>
          </div>
          {state.showPages == 'liveboard' && <LiveBoardScreen />}
          {state.showPages == 'hazardsIncidents' && <HazardsIncidentsScreen />}
          {state.showPages == 'safetyMeetings' && <SafetyMeetingsScreen />}
          {state.showPages == 'discussionTopics' && <DiscussionTopicsScreen />}
          {state.showPages == 'settings' && <SHSettingsScreen />}
          {state.showNewHazardsIncident && <CreateSHHazards />}
        </div>
      </SafetyHubAppContext.Provider>
    </>
  );
}
