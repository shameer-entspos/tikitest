...
import MainAssetManagerScreen from '@/components/Asset_Manager_App/Main_AssetManager_Screen';
import {
...
import { AMSettingsScreen } from '@/components/Asset_Manager_App/Settings/AssetManager_Settings';
import PendingServicesAndServiceGroup from '@/components/Asset_Manager_App/03_Servicing/PendingServiceAndServiceGroup';
import CheckInOutOptions from '@/components/Asset_Manager_App/01_CheckInOut/CheckInOutOptions';
import AssetManagerRecentActivity from '@/components/Asset_Manager_App/assetmanager_recent_activity';
import OrderItineraryScreen from '@/components/Asset_Manager_App/OrderItinerary/OrderItineraryScreen';
import ManageAssetsScreen from '@/components/Asset_Manager_App/04_ManageAssets/ManageAssetsScreen';
import CategoriesScreen from '@/components/Asset_Manager_App/05_Categories/CategoriesScreen';
import PendingServicesScreen from '@/components/Asset_Manager_App/03_Servicing/subpages/PendingServicesScreen';
import ServiceGroupScreen from '@/components/Asset_Manager_App/03_Servicing/subpages/ServiceGroupScreen';
import { CheckInScreen } from '@/components/Asset_Manager_App/01_CheckInOut/components/CheckIn';
import { CheckOutScreen } from '@/components/Asset_Manager_App/01_CheckInOut/components/CheckOut';
...
function Page({ params }: any) {
  ...
  // Open the correct section when returning from detail (e.g. ?open=manage or ?open=order)
  useEffect(() => {
        ...
        <div className="absolute inset-0 top-16 z-10 mx-auto h-[calc(var(--app-vh)-70px)] w-full max-w-[1360px] overflow-y-hidden bg-white p-4">
          <div className="breadCrumbs flex justify-between p-2">
            <div className="flex justify-start gap-5">
              ...
              <ul className="breadcurmb flex items-center">
          ...
          <div className="flex w-full flex-col gap-4 border-t-2 border-[#EEEEEE] md:flex md:flex-row">
            {state.showPages == 'checkinout' ? (
              <CheckInOutOptions />
            ) : state.showPages == 'servicing' ? (
              <PendingServicesAndServiceGroup />
            ) : (
              <MainAssetManagerScreen
            ...
            <div className="max-[664px] h-[calc(var(--app-vh)-70px)] w-full overflow-auto scrollbar-hide">
              {(state.showPages == undefined ||
                state.showPages == 'checkinout') && (
                <AssetManagerRecentActivity />
              )}
            </div>
          </div>

          {state.showPages == 'checkinout' && <CheckInOutOptions />}
          {state.showPages == 'show_checkin' && <CheckInScreen />}
          {state.showPages == 'show_checkout' && <CheckOutScreen />}

          {state.showPages == 'servicing' && <PendingServicesAndServiceGroup />}
          {state.showPages == 'pending_services' && <PendingServicesScreen />}
          {state.showPages == 'service_groups' && <ServiceGroupScreen />}

          {state.showPages == 'order' && <OrderItineraryScreen />}
          {state.showPages == 'manage' && <ManageAssetsScreen />}
          {state.showPages == 'categories' && <CategoriesScreen />}
          {state.showPages == 'settings' && <AMSettingsScreen />}
        </div>
