...
import {
  initialTimeSheetAppState,
  TimeSheetAppContext,
  TimeSheetAppContextProps,
  TimeSheetAppsReducer,
} from '../timesheet_context';
import TimeSheetRecentActivity from '@/components/TimeSheetApp/timesheet_recent_activity';
import TimeSheetView from '@/components/TimeSheetApp/TimeSheets_View';
import TimeSheetReviewSubmission from '@/components/TimeSheetApp/TimeSheet_Review_Submission';
import TimeSheetReportExport from '@/components/TimeSheetApp/TimeSheet_Report_Export';
import ExpensesView from '@/components/TimeSheetApp/Expenses_View';
import { ReportExpenseList } from '@/components/TimeSheetApp/Report_Expense_List';
import { ReportTimeSheetList } from '@/components/TimeSheetApp/Report_TimeSheet_List';
import { TimesheetSetting } from '@/components/TimeSheetApp/TimeSheet_Settings/Timesheet_Settings';
import MainTimeSheetScreen from '@/components/TimeSheetApp/Main_TimeSheet_Screen';
...
function Page({ params }: any) {
  ...
  const contextTimeSheetValue: TimeSheetAppContextProps = {
    state,
    dispatch,
  };
  ...
  const handleButtonClick = (
    pageType: 'timesheet' | 'expenses' | 'review' | 'report' | 'settings'
  ) => {
    dispatch({
  ...
  useEffect(() => {
    if (params.appid) {
      dispatch({
        type: TIMESHEETTYPE.ASSIGN_APP_ID,
        appId: params.appid,
      });
    }
  }, [params.appid]);
  return (
    <>
      <TimeSheetAppContext.Provider value={contextTimeSheetValue}>
        <div className="absolute inset-0 top-16 z-10 mx-auto h-[calc(var(--app-vh)-70px)] w-full max-w-[1360px] overflow-y-hidden bg-white p-4">
          ...
          <div className="flex w-full flex-col md:flex md:flex-row">
            {state.showPages == 'review' ? (
              <TimeSheetReviewSubmission />
            ) : state.showPages == 'report' ? (
            ...
            <div className="max-[664px] h-[calc(var(--app-vh)-70px)] w-full overflow-auto scrollbar-hide">
              {(state.showPages == undefined ||
                state.showPages == 'report' ||
                state.showPages == 'review') && <TimeSheetRecentActivity />}
            </div>
          </div>

          {state.showPages == 'timesheet' && <TimeSheetView />}
          {state.showPages == 'expenses' && <ExpensesView />}
          {state.showPages == 'report_expense' && <ReportExpenseList />}
          {state.showPages == 'report_timesheet' && <ReportTimeSheetList />}
          {state.showPages == 'settings' && <TimesheetSetting />}
        </div>
      </TimeSheetAppContext.Provider>
    </>
  );
}
