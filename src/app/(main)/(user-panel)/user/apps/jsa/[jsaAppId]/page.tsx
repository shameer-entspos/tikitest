...
import RecentActivity from '@/components/JobSafetyAnalysis/recentActivity';
import PPESafetyGear from '@/components/JobSafetyAnalysis/PPESafetyGear';
import HazardsListing from '@/components/JobSafetyAnalysis/HazardsListing';
import {
  JSAAppContext,
  JSAAppContextProps,
  JSAappsReducer,
  initialJSAAppState,
} from '@/app/(main)/(user-panel)/user/apps/jsa/jsaContext';
import { JSAAPPACTIONTYPE } from '@/app/helpers/user/enums';
import DraftsListing from '@/components/JobSafetyAnalysis/DraftListing';
import SubmissionListing from '@/components/JobSafetyAnalysis/SubmissionListing';
import TemplateListing from '@/components/JobSafetyAnalysis/TemplateListing';
import ActivityLog from '@/components/JobSafetyAnalysis/ActivityLog';
import CreateNew from '@/components/JobSafetyAnalysis/CreateNew';
import JSAAppSection from '@/components/JobSafetyAnalysis/JSA_APP_SECTION';
import JSADeleteModal from '@/components/JobSafetyAnalysis/Modal/JSA_Delete_Modal';
import JSAMailModel from '@/components/JobSafetyAnalysis/Modal/JSA_Mail_Model';
import JSAChatModel from '@/components/JobSafetyAnalysis/Modal/JSA_Chat_Model';
import JSADuplicateModal from '@/components/JobSafetyAnalysis/Modal/Duplicate_Model';
import { useRouter } from 'next/navigation';
import JSASettings from '@/components/JobSafetyAnalysis/JSA_Settings/JSA_Settings';
import JSAMultiDeleteModal from '@/components/JobSafetyAnalysis/Modal/JSA_Delete_Modal_Multiple';
import PPEDeleteModal from '@/components/JobSafetyAnalysis/Modal/PPE_Delete_Modal';

function Page({ params }: any) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [state, dispatch] = useReducer(JSAappsReducer, initialJSAAppState);
  const contextJSAValue: JSAAppContextProps = {
    state,
    dispatch,
  };
  ...
  useEffect(() => {
    ...
    if (open === 'submission' || open === 'drafts') {
      dispatch({
  ...
  useEffect(() => {
    if (params.jsaAppId) {
      dispatch({
        type: JSAAPPACTIONTYPE.SET_APPID,
        jsaAppId: params.jsaAppId,
      });
    }
  }, [params.jsaAppId]);
  return (
    <>
      <JSAAppContext.Provider value={contextJSAValue}>
        <div
          className={`absolute inset-0 top-16 z-10 mx-auto w-full max-w-[1360px] bg-white p-4 ${'h-[calc(var(--app-vh)-70px)] overflow-y-hidden'}`}
        >
          <div className="flex justify-between border-b-2 border-[#eeeeee] pb-[15px]">
            <div className="flex justify-start gap-5">
              ...
              <ul className="breadcurmb flex items-center gap-[10px]">
                <li className="text-base font-semibold text-[#757575]">
                  Tiki Apps
                </li>
                <li>
                  <svg
          ...
          <div className="flex w-full flex-col md:flex md:flex-row">
            <JSAAppSection
              handleButtonClick={handleButtonClick}
              handleGoBack={handleGoBack}
            />

            <div className="max-[664px] h-[calc(var(--app-vh)_-_70px)] w-full overflow-auto scrollbar-hide">
              {state.showPages == undefined && <RecentActivity />}
            </div>
          </div>
          {state.showPages == 'createNew' && <CreateNew />}
          {state.showPages == 'ppeSafety' && <PPESafetyGear />}
          {state.showPages == 'hazards' && <HazardsListing />}
          {state.showPages == 'drafts' && <DraftsListing />}
          {state.showPages == 'submission' && <SubmissionListing />}
          {state.showPages == 'templates' && <TemplateListing />}
          {state.showPages == 'activityLog' && <ActivityLog />}
          {state.showPages == 'settings' && <JSASettings />}
          {state.showDuplicateModel && <JSADuplicateModal />}
          {state.showChatModel && <JSAChatModel />}
          {state.showMailSubmissionModel && <JSAMailModel />}
          {state.showSubmissionDeleteModel && <JSADeleteModal />}
          {Array.isArray(state.showMultiSubmissionDeleteModal) &&
            state.showMultiSubmissionDeleteModal.length > 0 && (
              <JSAMultiDeleteModal />
            )}
          {state.showPPEDeleteModel && <PPEDeleteModal />}

          {/* {state.showHazardsDeleteModel && <HazardsDeleteModal />} */}
        </div>
      </JSAAppContext.Provider>
    </>
  );
}
