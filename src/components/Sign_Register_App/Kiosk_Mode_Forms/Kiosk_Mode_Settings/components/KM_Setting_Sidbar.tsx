...
export function KMSidebar() {
  ...
  const stepsList = ["Default Settings", "Printer Settings"];
      ...
      <ul className="px-1">
        {stepsList.map((step, index) => (
          <li
            ...
            onClick={() => {
              context.dispatch({
                type: SR_APP_ACTION_TYPE.SETTING_TAB_KIOSK_MODE,
                selectedSettingTabKioskMode: step,
              });
            }}
            className={`${
              context.state.selectedSettingTabKioskMode == step
