...
interface SiteMultiSelectModalProps {
  handleShowModel: () => void;
  selectedSubmissions: Site[];
}

const MultiSiteDeleteModel: React.FC<SiteMultiSelectModalProps> = ({
  handleShowModel,
  selectedSubmissions,
}) => {
  const axiosAuth = useAxiosAuth();
  const queryClient = useQueryClient();
  const deleteMultipleMutation = useMutation(deleteManySites, {
    onSuccess: () => {
      handleShowModel();
            ...
            <ModalHeader className="flex flex-row items-start gap-2 px-5 py-5">
              ...
              <div>
                ...
                <span className="mt-1 text-base font-normal text-[#616161]">
                  Are you sure you want to delete all sites? This action can not
                  be undone.
                </span>
              </div>
            </ModalHeader>
              ...
              <Button
                ...
                onClick={() => {
                  deleteMultipleMutation.mutate({
                    axiosAuth,
                    data: {
                      ids: selectedSubmissions.map((site) => site._id),
                    },
                  });
                }}
              >
                {deleteMultipleMutation.isLoading ? <Loader /> : <>Delete</>}
              </Button>
            </ModalFooter>
          </>
        )}
...
export default MultiSiteDeleteModel;
