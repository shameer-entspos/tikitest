...
const ShowExpanseDetail = ({
  ...
  const rawImages = (
    context.state.selectedExpanse?.model?.images ?? []
  ...
  const updateStatusMutation = useMutation(updateReviewStatus, {
    onSuccess: (response) => {
      const data = response.data as Expanse;
  ...
  return (
    ...
    >
      <ModalContent className="max-w-[600px] rounded-3xl bg-white">
        {(onCloseModal) => (
          <>
            <ModalHeader className="flex flex-row items-start gap-2 border-b border-[#E2F3FF] px-5 py-5">
              <svg
                width="50"
                height="50"
                viewBox="0 0 50 50"
              ...
              <div>
                ...
                <span className="mt-1 text-base font-normal text-[#616161]">
                  {'Review expense details below.'}
                </span>
              </div>
            </ModalHeader>
            <ModalBody className="">
              <div className="h-[60vh] max-h-[520px] overflow-y-scroll px-8">
                <div className="flex w-full justify-between">
                  <div className="flex flex-col">
                    ...
                    <span className="flex gap-2 py-2">
                      <PresignedUserAvatar
                        photo={
                          context.state.selectedExpanse?.model?.createdBy?.photo
                        }
                        fallback="/images/user.png"
                        alt="avatar"
                        className="h-8 w-8 rounded-full border border-gray-500 text-[#616161]"
                      />
                      {`${context.state.selectedExpanse?.model?.createdBy.firstName} ${context.state.selectedExpanse?.model?.createdBy.lastName}`}
                    </span>
                    <div className="flex items-center gap-4 py-3">
                      <span className="flex items-center gap-2">
                        ...
                        {`${dateFormat(
                          context.state.selectedExpanse?.model?.createdAt.toString() ??
                            ''
                        )}`}
                      </span>
                      <span className="flex items-center gap-2">
                  ...
                  {from === 'review' && (
                    <TimeSheetAllReviewButton
                      status={context.state.selectedExpanse?.model?.status}
                      handleStatus={handleStatus}
                    />
                  )}
                </div>
                <div className="pt-4">
                  <h6 className="py-1 text-sm">Expense ID</h6>
                  <span className="font-semibold">
                    {context.state.selectedExpanse?.model?.referenceId}
                  </span>
                </div>
                <div className="pt-4">
                  <h6 className="py-1 text-sm">Invoice Value</h6>
                  <span className="font-semibold">{`${context.state.selectedExpanse?.model?.invoiceValue} `}</span>
                </div>

                <div className="relative pt-4">
                  <h6 className="py-1 text-sm">Assigned Projects</h6>
                  <div
                    className={
                  ...
                  >
                    {context.state.selectedExpanse?.model?.projects[0].name}
                    {(context.state.selectedExpanse?.model?.projects ?? [])
                      .length > 1
                      ? ` +${
                          (context.state.selectedExpanse?.model?.projects ?? [])
                            .length - 1
                ...
                <div className="max-w-xs pt-4">
                  ...
                  <UserCard
                    ...
                    index={0}
                    name={context.state.selectedExpanse?.model?.customer}
                  />
                </div>
                <div className="max-w-xs pt-4">
                  <div className="text-[14px] text-[#616161]">
                  ...
                  <UserCard
                    submittedBy={session?.user.user}
                    index={0}
                    name={context.state.selectedExpanse?.model?.supplier}
                  />
                </div>
                <div className="max-w-xs pt-4">
                  <h6 className="py-1 text-sm">Description</h6>
                  <span className="whitespace-normal break-words font-normal">
                    {`${context.state.selectedExpanse?.model?.description}`}
                  </span>
                </div>
                <div className="pt-4">
                  <h6 className="py-1 text-sm">Invoice / Reference</h6>
                  <span className="font-semibold">{`${context.state.selectedExpanse?.model?.reference}`}</span>
                </div>

                <div className="pt-4">
                  {(context.state.selectedExpanse?.model?.images ?? []).length >
                    0 && <h6 className="py-1 text-sm">Attachments</h6>}

                  {(context.state.selectedExpanse?.model?.images ?? []).map(
                    (url, index) => {
                      const displayUrl = resolvedUrls?.[index] ?? url;
                      return (
                        <div
                          key={index}
                        ...
                        >
                          <div className="flex items-center truncate py-1">
                            <img
                              src={displayUrl}
                              alt={`Image ${index}`}
                              className="mx-2 h-8 w-8 rounded-lg object-cover"
                            />
                          ...
                          <div
                            ...
                            onClick={() => {
                              ...
                              link.download = getLastSegment(url); // Use the image name for download
            ...
            <ModalFooter className="flex justify-center gap-12 border-t-2 border-gray-200">
              ...
              <Button
                isDisabled={
                  context.state.selectedExpanse?.model?.status === 'review' ||
                  context.state.selectedExpanse?.model?.status === 'approved'
                }
                className={
                  context.state.selectedExpanse?.model?.status === 'review' ||
                  context.state.selectedExpanse?.model?.status === 'approved'
                ...
                onPress={() => {
                  if (
                    ...
                    context.dispatch({
                      ...
                      selectedExpanse: {
                        showAs: 'edit',
                        model: context.state.selectedExpanse?.model,
                      },
                    });
                  }
                }}
...
export default ShowExpanseDetail;
