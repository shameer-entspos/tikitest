...
} from './TimeSheet_Helper_Component';
...
import ExpenseReviewMultiSelectModal from './Models/MultiSelectModels/ReviewExpenseMultiSelectModel';
...
function ExpenseReviewListView() {
  ...
  const [selected, setSelected] = useState<'approved' | 'review' | 'not'>(
    'not'
  );
  ...
  const handleSelect = (option: 'approved' | 'review' | 'not') => {
    setSelected(option);
    setIsOpen(false);
  };
  ...
  // Submitted By: when projects are selected, only show users assigned to those projects
  const submittedByUserOptions = useMemo(() => {
    const allUsers = (users ?? []).filter((u) => u.role !== 4 && u.role !== 5);
    if (!selectedProjects.length || !projects?.length) return allUsers;
    const projectIds = new Set(selectedProjects);
    const userIds = new Set<string>();
    projects.forEach((p) => {
      if (p._id && projectIds.has(p._id) && p.users?.length) {
        p.users.forEach((ue: { user?: { _id?: string } }) => {
          if (ue.user?._id) userIds.add(ue.user._id);
        });
      }
    });
    if (userIds.size === 0) return allUsers;
    return allUsers.filter((u) => u._id && userIds.has(u._id));
  }, [users, projects, selectedProjects]);

  const filterData = (data ?? [])
    .filter((ex) => ex.status == selected)
    .filter((forSearch) => {
      if (searchQuery == '') {
        return true;
      }
    ...
    .filter((forSubmittedBy) => {
      ...
      return (
        forSubmittedBy.createdBy._id &&
        selectedSubmittedBy.includes(forSubmittedBy.createdBy._id)
      );
    })
    .filter((forReference) => {
      if (!isApplyFilter) {
  ...
  const [selectedExpenses, setCheckedExpenses] = useState<Expanse[]>([]);

  // Handle "Select All" checkbox change
  const handleSelectAllChange = () => {
    if ((filterData ?? []).length == (selectedExpenses ?? []).length) {
      handleCancel();
    } else {
      setCheckedExpenses([...(filterData ?? [])]);
    }
  };

  const handleCheckboxChange = (ex: Expanse) => {
    if (selectedExpenses.some((selectedEx) => selectedEx._id === ex._id)) {
      setCheckedExpenses([
        ...(selectedExpenses ?? []).filter(
          (selectedEx) => selectedEx._id !== ex._id
        ),
      ]);
    } else {
      setCheckedExpenses([...(selectedExpenses ?? []), ex]);
    }
  };
  ...
  // Sorting function based on column and direction - default to newest first
  const sortedData = [...(filterData ?? [])].sort((a, b) => {
    if (sortColumn === 'expense_id') {
      const fieldA = a.referenceId.toLowerCase();
      const fieldB = b.referenceId.toLowerCase();
      if (fieldA < fieldB) return sortDirection === 'asc' ? -1 : 1;
      if (fieldA > fieldB) return sortDirection === 'asc' ? 1 : -1;
      return 0;
    } else {
      // expense_date - default to descending (newest first)
      const fieldA = new Date(a.createdAt).getTime();
      const fieldB = new Date(b.createdAt).getTime();
  ...
  /// selected item modal things

  const [selectedOption, setSelectedOption] = useState<
    'approve' | 'underReview' | 'notApproved' | 'edit' | 'deleteTimesheet'
  >('approve');
  ...
  const handleOptionChange = (
    option: 'approve' | 'underReview' | 'notApproved' | 'deleteTimesheet'
  ) => {
  ...
  return (
    <>
        ...
        <div className="flex h-full flex-1 flex-col justify-start overflow-auto scrollbar-hide">
          <div className="mt-4 flex flex-col justify-between gap-2 lg:flex-row">
            ...
            <div className="flex items-center gap-2">
              {/* DropDown Custom */}
              <div className="relative z-50 hidden text-left md:flex">
                <button
                  type="button"
                  className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-md border border-gray-300 bg-[#E2F3FF] px-3 py-1 text-sm font-medium text-gray-700 shadow-sm hover:bg-[#e1f0fa] focus:outline-none"
                  id="options-menu"
                  aria-expanded="true"
                  aria-haspopup="true"
                  onClick={handleToggleTopbar}
                >
                  {StatusMessages({ status: selected })}
                  <svg
                    width="12"
                    height="13"
                    viewBox="0 0 12 13"
                ...
                {isOpen && (
                  <div
                    className="absolute left-0 z-50 mt-2 w-36 origin-top-left rounded-md bg-[#E2F3FF] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                    role="menu"
                  ...
                  {(sortedData ?? []).map((item: Expanse, index: number) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {hoveredUser === index && (
                          <div className="pointer-events-none absolute top-2 z-20 w-fit rounded-lg border bg-gray-50 p-2 text-xs text-[#616161] shadow-lg">
                            <div className="flex items-start gap-2">
                              ...
                              <div className="space-y-2">
                                <p className="text-sm font-semibold text-[#605f5f]">{`${item.createdBy.firstName} ${item.createdBy.lastName}`}</p>
                      ...
                      <td
                        className="hidden p-2 sm:table-cell"
                      ...
                      <td className="p-2 text-left">
                        {TimeSheetStatusMessages({ status: selected })}
                      </td>
                      <td
          ...
          {sortedData?.length == 0 && isLoading == false && (
            <div className="mx-auto mt-[62px] flex w-full flex-grow flex-col items-center gap-4">
              <img
                src="/images/empty_tin_can.png"
      ...
      <Modal
        ...
        placement="auto"
        size="xl"
        className="absolute max-h-[700px] py-2 pl-8"
      >
        <ModalContent className="max-w-[600px] rounded-3xl bg-white">
          {(onCloseModal) => (
            <>
              ...
              <ModalBody className="mt-0 flex flex-col justify-start overflow-y-scroll p-0 pl-2 pr-6 pt-4 scrollbar-hide">
                ...
                <div className="mb-16 w-full">
                  <CustomSearchSelect
                    label="Submitted By"
                    data={submittedByUserOptions.map((user) => ({
                      label:
...
export default ExpenseReviewListView;
