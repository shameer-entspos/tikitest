...
import ReviewTimesheetMultiSelectModel from './Models/MultiSelectModels/ReviewTimesheetMultiSelectModel';
...
function TimeSheetReviewListView() {
  ...
  // Model, dropdown, and selection states
  ...
  const submittedByUserOptions = useMemo(() => {
    // Exclude customers (role 4) and suppliers (role 5) – only show internal users
  ...
  const filterData = (data ?? [])
    ...
    .filter((byRange) => {
      ...
      if (!isDateRangeApplied) return true;
    ...
    .filter((forSubmittedBy) => {
      if (selectedSubmittedBy.length === 0) return true;
  ...
  const handleSelectAllChange = () => {
    if ((filterData ?? []).length == (selectedTimesheets ?? []).length) {
      handleCancel();
    } else {
      setCheckedTimeSheets([...(filterData ?? [])]);
    }
  };
  ...
  // Topbar and dropdown toggle handlers
  const handleToggleTopbar = () => setIsOpen(!isOpen);
  ...
  const sortedData = [...(filterData ?? [])].sort((a, b) => {
    if (sortColumn === 'timesheet_id') {
      ...
      // timesheet_date - default to descending (newest first)
    ...
    <>
            ...
            <table className="mt-3 w-full border-collapse font-Open-Sans">
              <thead className="w-full text-[#616161]">
                <tr className="max-h-[40px] w-full text-left">
                  <th className="flex rounded-l-lg bg-[#F5F5F5] px-4 py-3 text-left text-[14px] text-sm font-normal">
                    Timesheet ID
                    <img
                  ...
                  {(sortedData ?? []).map((item: TimeSheet, index: number) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {hoveredUser === index && (
                          <div className="pointer-events-none absolute top-2 z-20 w-fit rounded-lg border bg-gray-50 p-4 text-xs text-[#616161] shadow-lg">
                      ...
                      >
                        ...
                        >
                          {item.projects[0]?.name ?? 'Not Assigned'}
                      ...
                      <td className="hidden p-2 lg:table-cell">{`${item.timeTracker.hours.toString().padStart(2, '0')}:${item.timeTracker.minutes.toString().padStart(2, '0')}`}</td>
      ...
      {context.state.selectedTimeSheet &&
        context.state.selectedTimeSheet.showAs == 'detail' && (
          <ShowTimeSheetDetail from="review" />
        )}

      {/* Selected Items Model */}
      {showSelectedItemsModal && (
        <ReviewTimesheetMultiSelectModel
          handleShowModel={() => {
...
export default TimeSheetReviewListView;
