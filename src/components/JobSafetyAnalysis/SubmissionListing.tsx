...
/** PDF generate API (express-tiki): pass JSA id, backend returns PDF from template */
const PDF_GENERATE_API_PATH = 'user/pdf/generate';
...
const SubmissionListing = () => {
  ...
  const handleViewPDF = async (item: JSAAppModel) => {
    if (!item) {
      toast.error('No submission data to generate PDF');
      return;
    }
    try {
      ...
      const response = await axiosAuth.get(
        `${PDF_GENERATE_API_PATH}?${params.toString()}`,
        { responseType: 'blob' }
      );
      toast.remove();
      ...
      try {
        toast.loading('Generating PDF...', { className: 'text-primary-500' });
        const blob = await pdf(<JSAPDF data={item} />).toBlob();
        toast.remove();
        const pdfUrl = URL.createObjectURL(blob);
        window.open(pdfUrl, '_blank', 'noopener,noreferrer');
        setTimeout(() => URL.revokeObjectURL(pdfUrl), 10000);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['JSASubmissions', adminMode],
    queryFn: () =>
      getJSASubmissionList({
        axiosAuth,
        isAdmin: adminMode,
  ...
  const filterData = (data ?? [])
    // .filter((app) => {
    //   if (!adminMode) {
    //     if (app.organizationId.email === session?.user.user.email) {
    //       return app;
    //     }
    //   } else return app;
    // })
    ...
    //     return app;
    //   } else if (
    //     app.organizationId._id == session?.user.user.organization?._id
    //   ) {
    //     return app;
    //   }
    // })
    .filter((forProjects) => {
      ...
      return forProjects.projectIds.some(
    ...
    .filter((forCustomer) => {
      ...
      if (selectedCustomers.length != 0) {
        ...
        console.log('inData: ', forCustomer.organizationId._id);
        return true;
      }

      return (
        forCustomer.organizationId._id &&
        selectedCustomers.includes(forCustomer.organizationId._id)
      );
    })
  ...
  const [selected, setSelected] = useState<'Last Modified' | 'Created Date'>(
    'Last Modified'
  );
  const handleSelect = (option: 'Last Modified' | 'Created Date') => {
    setSelected(option);
    setIsOpen(false);
  };

  const [sortColumn, setSortColumn] = useState<'jsa_name' | 'jsa_date'>(
    'jsa_date'
  );
  ...
  const handleSort = (column: 'jsa_name' | 'jsa_date') => {
  ...
  // Clear selection when bulk-delete modal is closed (cancel or after delete)
  const prevMultiDeleteModalRef = useRef(
    context.state.showMultiSubmissionDeleteModal
  );
  useEffect(() => {
    if (
      prevMultiDeleteModalRef.current &&
      !context.state.showMultiSubmissionDeleteModal
    ) {
      setCheckedSubmissions([]);
        ...
        <div className="flex h-full flex-1 flex-col justify-start overflow-auto scrollbar-hide">
          ...
          <div className="mb-4 mt-4 flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h2 className="ml-2 text-2xl font-bold">Submissions</h2>
            </div>
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="py-1" role="none">
                            <button
                              onClick={() => handleSelect('Last Modified')}
                            ...
                            <button
                              onClick={() => handleSelect('Created Date')}
                              className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50"
                              role="menuitem"
                            >
                              Created Date
          ...
          <table className="w-full border-collapse">
              ...
              <tr className="max-h-[40px] w-full">
                <th className="flex rounded-l-lg bg-[#F5F5F5] py-3 pl-4 text-left">
                  ...
                  <img
                    ...
                    className={`transform cursor-pointer px-2 ${
                      sortColumn === 'jsa_name' && sortDirection === 'desc'
                        ? 'rotate-180'
                        : ''
                    }`}
                    onClick={() => handleSort('jsa_name')}
                    alt="image"
                  />
                </th>
                ...
                <th className="hidden bg-[#F5F5F5] px-2 py-3 text-left md:flex">
                  ...
                  <img
                    src="/images/fluent_arrow-sort-24-regular.svg"
                    className={`transform cursor-pointer px-2 ${
                      sortColumn === 'jsa_date' && sortDirection === 'desc'
                        ? 'rotate-180'
                        : ''
                    }`}
                    onClick={() => handleSort('jsa_date')}
                    alt="image"
                  />
                </th>
            ...
            {!isLoading && (
              <tbody className="text-sm font-normal text-[#1E1E1E]">
                {(sortedData ?? []).map((item: JSAAppModel, index: number) => (
                  <tr
                    key={item._id}
                    className="relative border-b px-2 even:bg-[#F5F5F5]"
                  >
                    <td
                      className="cursor-pointer pl-4 font-medium text-[#0063F7] hover:underline"
                      onClick={() => {
                        router.push(
                          `/user/apps/jsa/${context.state.jsaAppId}/${item?._id}/${item?.saveAs ?? 'Submission'}?from=submissions`
                        );
                      }}
                    >
                      {item.name}
                    ...
                    <td className="hidden p-2 sm:table-cell">
                      <div
                        ...
                        onMouseEnter={() => setHoveredSharing(index)}
                        onMouseLeave={() => setHoveredSharing(null)}
                      >
                        {(item.projectIds ?? []).length > 0 &&
                          item.projectIds[0].name}
                        {item.projectIds.length > 1
                          ? ` +${item.projectIds.length - 1}`
                          : null}
                      </div>
                    </td>
                    ...
                    {/* </div> */}
                    <td className="hidden p-2 md:table-cell">{`${dateFormat(
                          ...
                          <Dropdown placement="bottom-end" className="">
                            ...
                            <DropdownMenu aria-label="Dynamic Actions">
                              ...
                              <DropdownItem
                                ...
                                onClick={() => {
                                  context.dispatch({
                                    type: JSAAPPACTIONTYPE.SHOW_CAHT_MODEL,
                                    showChatModel: item,
                                  });
                                }}
                              >
              ...
              <div className="flex w-fit justify-end gap-4 text-right">
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    e.stopPropagation();
                    if (
                      !Array.isArray(selectedSubmissions) ||
                      selectedSubmissions.length === 0
                    )
                      return;
                    context.dispatch({
                      type: JSAAPPACTIONTYPE.SHOW_MULTI_SUBMISSION_DELTE_MODEL,
                      showMultiSubmissionDeleteModel: selectedSubmissions,
                    });
                  }}
                >
              ...
              <ModalBody className="flex flex-col justify-start gap-8 overflow-y-scroll p-0 pb-16 pt-8 scrollbar-hide">
                ...
                <div className="w-full">
                  <CustomSearchSelect
                    ...
                    onSelect={(selected: any) => setSelectedProjects(selected)}
                    placeholder="All"
                  />
                </div>
                ...
                <div className="w-full">
                  <CustomSearchSelect
                    ...
                    data={(data ?? []).map((customer) => ({
                      label: customer.organizationId.name ?? '',
                      value: customer.organizationId._id ?? '',
                    }))}
                    showImage={true}
                    multiple={true}
                    isOpen={openDropdown === 'dropdown2'}
                    onToggle={() => handleToggle('dropdown2')}
                    onSelect={(selected: any) => setSelectedCustomers(selected)}
                    placeholder="All"
                  />
                </div>
