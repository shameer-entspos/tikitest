...
import RelatedSVGIcon from './RelatedSVGIcon/RelatedSVGIcon';
import { checkJSAPermission } from '@/app/(main)/(user-panel)/user/apps/api';
...
const JSAAppSection = ({
  ...
  const { data, isLoading } = useQuery('JSASettingPermission', () =>
    ...
    <>
      <div className="flex h-[calc(var(--app-vh)_-_151px)] w-full max-w-[668px] flex-col gap-5 border-[#EEEEEE] pt-5 sm:overflow-auto sm:pl-5 sm:pr-12 lg:border-r-2 xl:gap-8">
        <div className="grid w-full gap-5 sm:grid-cols-2">
          <div onClick={() => handleButtonClick('createNew')}>
            <MainPageContainer
              type="big"
              heading="Create New"
              description="Conduct a Job Safety Analysis for on or more of your projects."
              iconSvg={<RelatedSVGIcon type="createNew" />}
            />
          </div>

          <div onClick={() => handleButtonClick('submission')}>
            <MainPageContainer
              type="big"
              heading="Submissions"
              description="View your completed JSA submissions."
              iconSvg={<RelatedSVGIcon type="submissions" />}
            />
          </div>
        </div>
        ...
        <div className="grid-col-1 grid w-full gap-5 xl:grid-cols-2">
          <MainPageContainer
            ...
            type="small"
            heading="Drafts"
            iconSvg={<RelatedSVGIcon type="drafts" />}
          />
          ...
          <MainPageContainer
            ...
            type="small"
            heading="Templates"
            iconSvg={<RelatedSVGIcon type="templates" />}
          />
          <MainPageContainer
            onClick={() => handleButtonClick('ppeSafety')}
            type="small"
            heading="PPE & Safety Gear"
            iconSvg={<RelatedSVGIcon type="ppe" />}
          />

          <MainPageContainer
            onClick={() => handleButtonClick('hazards')}
            type="small"
            heading="Hazards & Risks"
            iconSvg={<RelatedSVGIcon type="hazards" />}
          />
          {(hasProjectManagePermission?.apps ?? false) && (
            <MainPageContainer
              ...
              type="small"
              heading="Settings"
              iconSvg={<RelatedSVGIcon type="settings" />}
            />
          )}
        ...
        <div
          ...
          onClick={() => {
            window.open(
              'https://tikiworkplace.com/app-marketplace/job-safety-analysis-jsa/',
              '_blank'
            );
          }}
        >
