...
export function SelectManagerAndSuperVisorList({
  handleShowCreate,
  data,
  showSearchBox = true,
}: {
  ...
  const filteredAndSortedUsers = useMemo(() => {
    ...
    const users = (data ?? [])
      ...
      .filter((user) => user.email !== session?.user.user.email)
      .filter((user) => {
        if (!showSearchBox || !query) return true;
    ...
    return sorted;
  }, [data, searchQuery, selectedOption, session?.user.user.email, showSearchBox]);
  ...
  return (
    <>
      ...
      <div className="px- h-[150px] overflow-scroll scrollbar-hide md:h-[350px]">
        <div
          className={`my-2 flex items-center justify-between rounded-lg border border-gray-300 px-3 py-3 text-sm ${(context.state.jsaDetailSelectedManagers ?? []).length == 3 ? 'disabled opacity-50' : ''}`}
        >
          ...
          <CustomBlueCheckBox
            label="Me"
            disabled={
              (context.state.jsaDetailSelectedManagers ?? []).length == 3
            }
            checked={context.state.jsaDetailSelectedManagers?.some(
              (id) => id.email === session?.user.user.email
            )}
            onChange={() => {
              context.dispatch({
