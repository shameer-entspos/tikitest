import { getAllPPEs, PPEModel } from '@/app/(main)/(user-panel)/user/apps/api';
...
export function SelectPPEINSTEPS({
  handleShowCreate,
  handlePPESelection,
  isPPESelected,
}: {
  handleShowCreate: () => void;
  handlePPESelection: ({ PPEs }: { PPEs: PPEModel }) => void | PPEModel[];
  isPPESelected: ({ PPEs }: { PPEs: PPEModel }) => boolean;
}) {
  ...
  return (
    <>
      ...
      <div className="h-[250px] overflow-scroll scrollbar-hide md:h-[500px]">
        {isLoading ? (
          <Loader />
        ) : (
          <>
            {(filterProjects ?? []).map((PPEs) => {
              return (
                <div
                  className="my-2 flex items-center justify-between rounded-lg border border-gray-300 px-3 py-3"
                  key={PPEs._id}
                >
                  <span className="">{PPEs.name}</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <CustomBlueCheckBox
                      label="Me"
                      checked={isPPESelected({ PPEs })}
                      onChange={() => {
                        handlePPESelection({ PPEs });
                      }}
                    />
                  </div>
                </div>
