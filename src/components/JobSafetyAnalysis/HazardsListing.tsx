...
import NewHazardsModal from './Modal/NewHazardsModal';
import DetailHazardModal from './Modal/DetailHazardModal';
...
import DeleteHazardModel from './Modal/DeleteHazardModel';
...
import HazardsMultiDeleteModal from './Modal/Hazards_Delete_Modal_Multiple';
const HazardsListing = () => {
  ...
  // useEffect(()=>{
  //   let hazards=[]
  //   if(selected.toLowerCase() == 'my list'){
  //     hazards = data?.filter(hazard=>hazard.sharing === 1)!
  //     setHazardsList(hazards);
  //   }
  //   else if(selected.toLowerCase() == 'shared list'){
  //     hazards = data?.filter(hazard=>hazard.sharing === 2)!
  //   }
  //   else{
  //     hazards = data!;
  //   }
  ...
  const handleRowClick = (item: HazardModel) => {
  ...
  const filterData = data
    ?.filter((hazard) => {
      if (selected === 'Shared List') {
        return hazard.sharing === 2;
      }
      if (selected === 'My List') {
        return hazard.sharing === 1;
      } else {
        return true;
      }
    })
  ...
  const [selectedSubmissions, setCheckedSubmissions] = useState<HazardModel[]>(
    []
  );
  ...
  const handleCheckboxChange = (js: HazardModel) => {
    ...
    <>
      <div className="absolute inset-0 z-10 flex h-[calc(var(--app-vh)-70px)] w-full max-w-[1360px] flex-col bg-white px-4 pt-4 font-Open-Sans">
            ...
            <div className="sticky top-0 z-20 bg-white pb-2">
              <div className="mt-4 flex flex-col justify-between gap-2 lg:flex-row">
                ...
                <div className="flex items-center gap-2 text-2xl font-bold text-[#1E1E1E]">
                  ...
                  <h2 className="ml-2 text-2xl font-bold">Hazards & Risks</h2>
                </div>
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="py-1" role="none">
                          ...
                          <button
                            onClick={() => handleSelect('Shared List')}
                            className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50"
                            role="menuitem"
                          >
                            Shared List
            ...
            <table className="w-full border-collapse">
              <thead className="text-sm font-normal text-[#616161]">
                <tr className="max-h-[40px] w-full">
                  <th className="flex rounded-l-lg bg-[#F5F5F5] px-2 py-3 text-left">
              ...
              <tbody className="text-sm font-normal text-[#1E1E1E]">
                <>
                  {sortedData.map((item) => (
                    <tr
                      key={item._id}
                      className="relative overflow-auto border-b even:bg-[#F5F5F5]"
                    ...
                    >
                      ...
                      <td className="hidden p-2 sm:table-cell">
                        ...
                        >
                          <img
                            ...
                            alt={
                              item.sharing === 1 ? 'user icon' : 'people icon'
                            }
                          />
                          {item.sharing === 1 ? 'My List' : 'Shared List'}
                        </div>
                      </td>
                      ...
                      >
                        ...
                        {hoveredUser === item._id && (
                          <div className="w-128 absolute top-8 z-20 mt-2 rounded-lg border bg-gray-200 p-4 shadow-lg">
        ...
        <div className="absolute bottom-24 left-0 flex w-full justify-end p-4">
          ...
          {context.state.showModal === 'deleteModal' && (
            <DeleteHazardModel isOpen={true} onClose={handleClose} />
          )}
          {context.state.showModal === 'detailModal' && (
            <DetailHazardModal isOpen={true} onClose={handleClose} />
          )}
          {(context.state.showModal === 'NewModal' ||
            context.state.showModal === 'editModal' ||
            context.state.showModal === 'duplicateModel') && (
            <NewHazardsModal isOpen={true} onClose={handleClose} />
          )}
        </div>
            ...
            {isSelectMode && (
              <div className="flex w-fit justify-end gap-4 text-right">
                ...
                <Button
                  ...
                  onClick={() => {
                    context.dispatch({
                      type: JSAAPPACTIONTYPE.SHOW_MULTI_HAZARDS_DELTE_MODEL,
                      showMultiHazardsDeleteModal: selectedSubmissions,
                    });
                  }}
                >
      ...
      <Modal
        ...
        placement="auto"
        size="xl"
        className="absolute h-[700px] overflow-x-visible px-8 py-2"
      >
      ...
      {context.state.showMultiHazardsDeleteModal && (
        <HazardsMultiDeleteModal
          handleClose={() => {
            context.dispatch({
              type: JSAAPPACTIONTYPE.SHOW_MULTI_HAZARDS_DELTE_MODEL,
            });
