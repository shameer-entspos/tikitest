...
import { WithSettingsSidebar } from './With_JSA_Settings_Sidebar';
import {
  Modal,
  ModalContent,
  ModalHeader,
...
import { CreatenewManagerForm } from '../CreateNewComponents/CreateNewManageForm';
import { SelectManagerAndSuperVisorList } from '../CreateNewComponents/Select_Manager_AND_Supervisor';
import { JSASelectAdminSearch } from './JSA_Setting_Admin_Search';
    ...
    () => (
      <>
        <div className="flex items-center justify-between border-b-2 border-gray-300 pb-2">
          <div className="flex items-center gap-2 text-[20px] font-bold text-[#1E1E1E]">
            <img
              src="/svg/jsa/logo.svg"
              alt="show logo"
      ...
      <div className="absolute inset-0 top-4 z-10 h-full w-full max-w-[1360px] bg-white p-4 pt-0">
        ...
        <WithSettingsSidebar>
          {context.state.selectedSettingTab === 'Notifications' ? (
            <>
              <NotificationSection handleShowModel={handleShowModel} />
            </>
          ) : (
            <>
              <PermissionSection />
...
export default JSASettings;

function NotificationSection({ handleShowModel }: { handleShowModel: any }) {
  const context = useJSAAppsCotnext();
  const [created, setCreatedNotify] = useState(false);
  const [deleted, setDeletedNotify] = useState(false);
  ...
  useEffect(() => {
    if (notificationData) {
      setCreatedNotify(notificationData.notifyMeWhenCreated);
      setDeletedNotify(notificationData.notifyMeWhenDeleted);
      setTikiAIAssistant(notificationData.tikiAIAssistant ?? false);
      setEmailNotification(notificationData.emailNotification ?? false);
      // sync context managers so modal shows current recipients
      context.dispatch({
        type: JSAAPPACTIONTYPE.SET_JSA_DETAIL_MANAGERS,
        jsaDetailSelectedManagers: notificationData.recipients ?? [],
      });
    }
  ...
  const handleSubmit = ({
    ...
    if (!notificationData?._id) return;
    const data = {
      recipients: recipients
        ...
        : ((notificationData?.recipients ?? []).map((data) => ({
            firstName: data.firstName,
            lastName: data.lastName,
            email: data.email,
          })) ?? []),
      notifyMeWhenCreated:
        created ?? notificationData?.notifyMeWhenCreated ?? false,
      notifyMeWhenDeleted:
        deleted ?? notificationData?.notifyMeWhenDeleted ?? false,
      tikiAIAssistant: tikiAI ?? notificationData?.tikiAIAssistant ?? false,
      emailNotification: email ?? notificationData?.emailNotification ?? false,
      forceSubmission: notificationData?.forceSubmission ?? 'no',
      adminTeams:
        (notificationData?.adminTeams ?? []).map((data) => data) ?? [],
    };
  ...
  const handleUpdateRecipients = (recipients: Recipient[]) => {
    // Update context
    const userDetails = recipients.map(
      (r) =>
        ({
          _id: r._id || '',
          firstName: r.firstName,
    ...
    context.dispatch({
      type: JSAAPPACTIONTYPE.SET_JSA_DETAIL_MANAGERS,
      jsaDetailSelectedManagers: userDetails,
    });
    // Update settings - recipients already have only firstName, lastName, email
    handleSubmit({ recipients });
  };
  if (isLoading) {
    return <Loader />;
  }
  ...
  return (
    <>
      <div className="mx-4 my-4 flex max-h-[668px] w-4/6 flex-col overflow-y-auto rounded-lg border-2 border-[#EEEEEE] p-6 shadow md:w-5/6">
        ...
        <NotificationRecipients
          recipients={(notificationData?.recipients ?? []).map((user: any) => ({
            email: user.email || '',
            firstName: user.firstName || '',
            lastName: user.lastName || '',
          }))}
          onUpdateRecipients={handleUpdateRecipients}
          queryKey="listofUsersForJSA"
          showMaxLabel={true}
          recipientDisplayFormat="name-email"
        ...
        <div className="mt-8 flex flex-col">
          ...
          <div className="mt-2 grid grid-cols-[auto,1fr] items-start gap-2">
            <input
              ...
              disabled={isSaving}
              onChange={() => {
                handleSubmit({ created: !created, deleted });

                setCreatedNotify(!created);
              }}
            />
            <span className="text-gray-700">Submission has been created.</span>
          </div>
          <div className="mt-2 grid grid-cols-[auto,1fr] items-start gap-2">
            <input
              type="checkbox"
              ...
              disabled={isSaving}
              onChange={() => {
                handleSubmit({ created, deleted: !deleted });
                setDeletedNotify(!deleted);
              }}
            />
            <span className="text-gray-700">Submission has been deleted.</span>
          </div>
        </div>
        {/* Notification Methods  */}
        <div className="mt-8 flex flex-col">
...
function PermissionSection() {
  return (
    <>
      <div className="mx-4 my-4 flex h-[450px] max-h-[668px] w-4/6 flex-col rounded-lg border-2 border-[#EEEEEE] p-6 shadow md:w-5/6">
        <JSASelectAdminSearch />
      </div>
    </>
  );
}
