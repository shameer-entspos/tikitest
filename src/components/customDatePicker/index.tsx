...
import './../../components/JobSafetyAnalysis/CreateNewComponents/calender_style.css';
...
export default function CustomDateRangePicker({
  ...
  return (
    <>
      <label htmlFor="" className="flex items-end justify-between text-base">
        <label
          htmlFor=""
          className="cursor-pointer text-sm font-medium text-[#1E1E1E]"
        >
          {title}
      ...
      <div
        className={`relative mt-2 flex h-14 w-full cursor-pointer items-center justify-between rounded-md border-2 px-3`}
        onClick={() => {
      ...
      >
        <div className="text-sm">
          {selectedDate ? (
            <>
              {dateFormat(selectedDate.toString())}
      ...
      <div>
        {showDialog && (
          <ul className="absolute z-10 my-3 mt-1 w-full border-2 bg-white shadow-xl">
            <CalendarComponent
...
const CalendarComponent: React.FC<CustomCalendarProps> = ({
  ...
  const handleChange = (value1: Value) => {
    if (value1 instanceof Date) {
      // Single date selected
      //   setValue(value1); // Store the single selected date
      setActiveStartDate(value1); // Update the calendar's active start date
    } else if (Array.isArray(value1) && value1.length === 2) {
      // Range selected (not relevant if range selection is disabled)
      //   setValue(value1[0]); // Use the start of the range
      setActiveStartDate(value1[0]); // Set the active start date to the start of the range
    } else {
      // No date selected (value1 is null)
      //   setValue(null);
    }
  };
  ...
  return (
    <div className="flex justify-center">
      <div className="w-2/3">
        ...
        <div className="flex justify-between gap-4 px-2 py-4 font-Open-Sans text-[#0063F7]">
            ...
            <button
              ...
              onClick={() => {
                if (activeStartDate instanceof Date) {
                  // If a single date is selected, call onConfirm with the selected date
                  onConfirm(activeStartDate);
                } else if (
                  Array.isArray(activeStartDate) &&
                  activeStartDate[0] &&
                  activeStartDate[1]
                ) {
                  // If a date range is selected (just in case), call onConfirm with the start and end dates
                  //   onConfirm(value[0], value[1]);
                } else {
                  // Handle the case where no valid value is selected
