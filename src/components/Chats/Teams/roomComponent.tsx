...
export function DefaultAccordionBody({ room }: { room: TeamRooms }) {
  ...
  return (
    <div className="px-1">
      <div
        ...
        onClick={() => {
          chatSocket.emit('joinRoom', room.senderId, room._id, 'teams');
      ...
      >
        <div>
          ...
          >
            <svg
              ...
              className="h-[44px] w-[44px] rounded-full bg-primary-50/70"
              viewBox="0 0 50 48"
              fill="none"
        ...
        <div className="hidden w-[90%] flex-col justify-between md:pr-2 lg:flex">
          ...
          <div className="flex justify-between">
            ...
            <span className="mt-1 hidden text-xs text-gray-600 md:block">
              Teams
            </span>
          </div>
        </div>
...
export function TeamRoomsActivityList({
  groupedTeamRooms,
}: {
  groupedTeamRooms: GroupedRooms;
}) {
  ...
  return (
    <>
      {Object.keys(groupedTeamRooms).map((teamName: string) => {
        return (
          <div key={teamName}>
            {(groupedTeamRooms[teamName] ?? [])?.map((room: TeamRooms) => {
...
export function TeamRoomsList({
  ...
  return (
    <>
      {Object.keys(groupedTeamRooms).map((teamName: string) => (
        <Accordion open={accordionState[teamName]} key={teamName}>
          <AccordionHeader
            onClick={() => handleClick(teamName)}
            className="ml-2 flex w-full items-center justify-start border-none py-0"
          >
            <svg
              className={`h-5 w-5 pb-1 ${accordionState[teamName] ? '' : '-rotate-90'}`}
              viewBox="0 0 15 15"
              fill="currentColor"
              xmlns="http://www.w3.org/2000/svg"
            >
            ...
            <span className="text-base font-normal text-gray-600">
              {groupedTeamRooms[teamName][0].teamDetails.name}
            </span>
          </AccordionHeader>
          <AccordionBody>
            {(groupedTeamRooms[teamName] ?? [])?.map((room: TeamRooms) => {
...
export function PinnedTeamChats({ pinnedRooms }: { pinnedRooms: TeamRooms[] }) {
  ...
  return (
    <Accordion open={openPinned}>
      ...
      >
        <svg
          className={`h-5 w-5 pb-1 ${openPinned ? '' : '-rotate-90'}`}
          viewBox="0 0 15 15"
          fill="currentColor"
          xmlns="http://www.w3.org/2000/svg"
        >
      ...
      <AccordionBody>
        {(pinnedRooms ?? [])?.map((room: TeamRooms) => {
          return <DefaultAccordionBody room={room} key={room._id} />;
        })}{' '}
      </AccordionBody>
    </Accordion>
