...
function ReceiveChatRequests() {
  ...
  return (
    <>
      ...
      {isSuccess && (
        <div className="flex h-auto flex-col gap-2 overflow-y-scroll pb-4 scrollbar-hide">
          {data.map((e: ChatRequestList) => {
            return <ReceiveChatRequestCard key={e._id} e={e} />;
          })}
        </div>
      )}
    </>
...
function ReceiveChatRequestCard({ e }: { e: ChatRequestList }) {
  const queryClient = useQueryClient();
  const [loader, setLoader] = useState('');
  const senderPhotoDisplay = usePresignedUserPhoto(e.sender?.photo);
      ...
      >
        <div className="flex w-full gap-2">
          ...
          <div className="w-full flex-col justify-between">
            ...
            <div className="mt-1 flex w-full items-center justify-between">
              <h1 className="w-28 truncate text-ellipsis text-xs font-medium text-gray-600">{`${e.sender?.email}`}</h1>
              <h1 className="text-xs text-gray-500">Recieved</h1>
            </div>
          </div>
        </div>
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-1 px-2">
            ...
            <h1 className="w-16 truncate text-xs text-gray-600">
              {e.sender?.organization.name}
            </h1>
          </div>
          <div className="mt-2 flex items-center justify-between space-x-1">
            <button
              type="button"
              className="font-base rounded-xl bg-gray-800 px-3 py-1 text-xs text-white"
              onClick={() => {
                actionOnChatRequest({
                  action: 'reject',
                  id: e._id,
            ...
            >
              {responedChatRequestMutation.isLoading && loader == 'reject' ? (
            ...
            <button
              type="button"
              className="font-base rounded-xl bg-primary-500 px-3 py-1 text-xs text-white"
              onClick={() => {
                actionOnChatRequest({
                  action: 'accept',
                  id: e._id,
            ...
            >
              {responedChatRequestMutation.isLoading && loader == 'accept' ? (
                <div className="mt-1 inline-block h-3 items-center">
                  <Loader />
                </div>
              ) : (
...
export { ReceiveChatRequests, ReceiveChatRequestCard };
