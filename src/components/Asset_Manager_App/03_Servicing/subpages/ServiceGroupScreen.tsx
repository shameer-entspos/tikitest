...
import ServiceGroupDetail from '../Service_Group_Component/Service_Group_Detail';
import CustomInfoModal from '@/components/CustomDeleteModel';
import AddGroupServiceModel from './Add_Service_Group';
...
export default function ServiceGroupScreen() {
  ...
  const [selectedModel, setGroupModel] = useState<
    { model?: GroupService; action: 'edit' | 'delete' | 'view' } | undefined
  >(undefined);
  const handleAction = (
    action:
      | { model?: GroupService; action: 'edit' | 'delete' | 'view' }
      | undefined
  ) => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: 'groupServices',
    queryFn: () => getGroupServiceList({ axiosAuth }),
    refetchOnWindowFocus: false,
  });
  const queryClient = useQueryClient();

  // Sync selectedModel with updated query data when query refetches
  useEffect(() => {
    if (selectedModel?.model?._id && selectedModel?.action === 'view' && data) {
      const updatedModel = data.find(
        (item) => item._id === selectedModel.model?._id
      );
      if (updatedModel) {
        // Only update if the model reference changed (React Query will return same ref if unchanged)
        if (updatedModel !== selectedModel.model) {
          setGroupModel({
            ...selectedModel,
            model: updatedModel,
          });
  ...
  const deleteManyGroupServiceMutation = useMutation(deleteManyGroupService, {
    onSuccess: () => {
      handleCancel();
      setMultiDelete(!showMultiDelete);
      queryClient.invalidateQueries('groupServices');
    },
  });
  //////
  const [isSelectMode, setIsSelectMode] = useState(false);
  const [selectedGroupServices, setCheckedGroupService] = useState<
    GroupService[]
  >([]);
  const [showMultiDelete, setMultiDelete] = useState(false);
  const handleCheckboxChange = (js: GroupService) => {
    if (selectedGroupServices.some((selectedTs) => selectedTs._id === js._id)) {
      setCheckedGroupService([
        ...(selectedGroupServices ?? []).filter(
          (selectedTs) => selectedTs._id !== js._id
        ),
      ]);
    } else {
      setCheckedGroupService([...(selectedGroupServices ?? []), js]);
    }
  };
  const handleCancel = () => {
    setCheckedGroupService([]);
  ...
  const handleSelectAllChange = () => {
    if ((data ?? []).length == (selectedGroupServices ?? []).length) {
      handleCancel();
    } else {
      setCheckedGroupService(data ?? []);
    }
  };
    ...
    .filter((p) => {
      if (isApplyFilter) {
        if (selectedManager.length > 0) {
          return (p.managers ?? [])
            .map((m) => m._id)
            .some((m) => selectedManager.includes(m));
        }
      }
      return p;
    })
    .filter((p) => {
      if (isApplyFilter) {
        if (selectedTagged.length > 0) {
          // Filter by customer - check if the service group's customer matches selected customers
          return selectedTagged.some((customerId) => {
            const customer = customers?.find((c) => c._id === customerId);
            if (!customer) return false;
            const customerName =
              customer.customerName ||
              `${customer.firstName} ${customer.lastName}`;
            return p.customer === customerName;
          });
        }
      }
      return p;
    ...
    <>
          ...
          {isLoading ? (
            ...
            <table className="mt-3 w-full border-collapse font-Open-Sans">
              <thead className="bg-[#F5F5F5] text-left text-sm font-semibold text-[#616161]">
                <tr>
                  <th className="hidden w-64 px-2 py-3 md:flex">
                    Group Name
                    <img
                  ...
                  <th className="px-2 py-3">Asset Count</th>
                  <th className="hidden px-2 py-3 md:table-cell">Customer</th>
            ...
            <div>
              {isSelectMode && (
                <div className="flex w-fit justify-end gap-4 text-right">
                  ...
                  >
                    <div>Delete ({(selectedGroupServices ?? []).length})</div>
                  </Button>
                </div>
              )}
            </div>
      ...
      {selectedModel?.action == 'delete' && (
        <CustomInfoModal
          ...
          handleClose={() => handleAction(undefined)}
          onDeleteButton={() => {
      ...
      {showMultiDelete && (
        <CustomInfoModal
          doneValue={
            deleteManyGroupServiceMutation.isLoading ? <Loader /> : 'Delete'
          }
          handleClose={() => {
            handleCancel();
            setMultiDelete(!showMultiDelete);
          ...
          onDeleteButton={() => {
            deleteManyGroupServiceMutation.mutate({
              axiosAuth,
              data: { ids: selectedGroupServices.map((ts) => ts._id) },
            });
          }}
          subtitle="Are you sure you want to delete selected Service Groups. All Service Schedules associated will also be deleted. This action cannot be undone."
          title={`Delete Service Group (${selectedGroupServices.length})`}
        />
      )}
      {/* filter model */}
      <Modal
