...
import MainCreateSafetyMeetings from './Create_SafetyMeeting/Main_Create_Safety_Meetings';
...
export default function SafetyMeetingsScreen() {
  ...
  const areFiltersApplied = () => {
    ...
    return validProjects.length > 0 || validSubmitBy.length > 0;
  };
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['safetyMeetings', adminMode],
    queryFn: () => getSafetyMeetings({ axiosAuth, isAdmin: adminMode }),
    enabled: true,
  });
  const handleGoBack = () => {
    dispatch({
  ...
  const handleSelectAllChange = () => {
    // Only select items created by the logged-in user
    const ownSubmissions = (sortedData ?? []).filter(
      (item) => item.submittedBy?._id === session?.user?.user?._id
    );
    const selectedOwnSubmissions = (selectedSubmissions ?? []).filter(
      (item) => item.submittedBy?._id === session?.user?.user?._id
    );
    // Check if all enabled items are selected by comparing IDs
    const allSelected =
      ownSubmissions.length > 0 &&
      ownSubmissions.every((item) =>
        selectedOwnSubmissions.some((selected) => selected._id === item._id)
      );
    if (allSelected) {
      handleCancel();
    } else {
      setCheckedSubmissions([...ownSubmissions]);
    }
  };
  const handleCancel = () => {
  ...
  const handleCheckboxChange = (js: SafetyMeetings) => {
    // Only allow selection of own submissions
    if (js.submittedBy?._id !== session?.user?.user?._id) {
      return;
    }
  ...
  const deleteMutation = useMutation(deleteSafetyMeeting, {
    onSuccess: () => {
      handleGoBack();
    },
  });
  ...
  const updateMutation = useMutation(updateMultipleSafetyMeeting, {
    onSuccess: () => {
      handleCancel();
  ...
  return (
    <>
      <div className="absolute inset-0 z-10 flex h-[calc(var(--app-vh)-70px)] w-full max-w-[1360px] flex-col bg-white pt-4 font-Open-Sans">
        {/* TopBar */}
        {/* {memoizedTopBar} */}
        ...
        <div className="flex h-full flex-1 flex-col justify-start overflow-auto scrollbar-hide">
          ...
          {isLoading ? (
            ...
            <table className="mt-3 w-full border-collapse font-Open-Sans">
              <thead className="bg-[#F5F5F5] text-left text-sm font-normal text-[#616161]">
                <tr>
                  <th className="w-200 hidden px-2 py-3 lg:table-cell">
                    <div
                  ...
                  <th className="px-2 py-3 font-normal text-[#1E1E1E]">
                    Assigned Projects
                  </th>
                  ...
                  <th className="hidden py-3 md:table-cell">
                    <div
                      className="flex cursor-pointer items-center gap-1 font-normal text-[#1E1E1E]"
                  ...
                  return (
                    <tr
                      ...
                      className={`relative border-b even:bg-[#F5F5F5] ${
                        isSelectMode && !isOwnSubmission
                          ? 'cursor-not-allowed opacity-50'
                          : ''
                      }`}
                    >
                      <td
                        className="w-400 hidden px-2 py-2 lg:table-cell"
                        onClick={() => {
                          ...
                          router.push(
                            `/user/apps/sh/${state.appId}/safetymeeting/${item._id}?token=${token}&from=safetyMeetings`
                          );
                        }}
                      >
                            ...
                            >
                              <input
                                ...
                                className={`h-5 w-5 appearance-none rounded-md border-2 ${
                                  !isOwnSubmission
                                    ? 'cursor-not-allowed border-[#9E9E9E] bg-gray-100 opacity-50'
                                    : selectedSubmissions.some(
                                          (ts) => item._id == ts._id
                                        )
                                      ? 'cursor-pointer border-[#6990FF] bg-[#6990FF] checked:border-[#6990FF] checked:bg-[#6990FF]'
                                      : 'cursor-pointer border-[#9E9E9E] bg-white'
                                } transition-colors duration-200 ease-in-out`}
                                ...
                                <DropdownItem
                                  ...
                                  onClick={() => {
                                    if (
                                      ...
                                      dispatch({
                                        type: SAFETYHUBTYPE.SHOW_SAFETY_MEETING_CREATE_MODEL_EDIT,
                                        selected_safety_meeting_model: item,
                                      });
                                    }
                                  }}
                                >
          ...
          <div className="fixed bottom-36 right-[16%] z-10">
            <Button
              ...
              {isSelectMode && (
                <div className="flex w-fit justify-end gap-4 text-right">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      // Filter to only include own submissions and open delete modal directly
                      const ownSubmissions = selectedSubmissions.filter(
                        (item) =>
                          item.submittedBy?._id === session?.user?.user?._id
                      );
      ...
      {state.show_safety_meeting_create_model && <MainCreateSafetyMeetings />}
      {deleteItemId && (
        <CustomInfoModal
          title={'Delete Hazard & Incident'}
          handleClose={() => setDeleteItemId(undefined)}
          onDeleteButton={() => {
            deleteMutation.mutate({ axiosAuth, id: deleteItemId });
          }}
          doneValue={
      ...
      {selectedAction === 'delete' && (
        <CustomInfoModal
          ...
          onDeleteButton={() => {
            // Filter to only delete own submissions
            const ownSubmissions = selectedSubmissions.filter(
              (item) => item.submittedBy?._id === session?.user?.user?._id
            );
            if (ownSubmissions.length > 0) {
              updateMutation.mutate({
                axiosAuth,
                data: {
                  ids: ownSubmissions.map((item: any) => item._id),
                  deletedAt: new Date(),
                },
              });
            }
          }}
          subtitle="Are you sure you want to delete this Safety Meeting. This action cannot be undone."
          title={`Delete (${selectedSubmissions.filter((item) => item.submittedBy?._id === session?.user?.user?._id).length}) Safety Meeting${selectedSubmissions.filter((item) => item.submittedBy?._id === session?.user?.user?._id).length > 1 ? 's' : ''}`}
        />
      )}
      ...
      {selectedAction === 'resolution' && (
        <Modal
          isOpen={true}
          onOpenChange={() => setSelectedAction(undefined)}
          placement="top-center"
          size="xl"
        >
          <ModalContent className="max-w-[600px] rounded-3xl bg-white">
                ...
                <ModalBody className="my-4">
                  <div className="mb-8 flex flex-col space-y-2 p-2">
                    ...
                    <textarea
                      ...
                      value={organizationForm.values.comments}
                      className={` ${
                        organizationForm.errors.comments &&
                        organizationForm.touched.comments
                          ? 'border-red-500'
                          : 'border-[#EEEEEE]'
      ...
      >
        <ModalContent className="max-w-[600px] rounded-3xl bg-white">
          {(onCloseModal) => (
            <>
              <ModalHeader className="flex flex-row items-center justify-between border-b-2 border-gray-200 px-1 py-5">
                <div>
