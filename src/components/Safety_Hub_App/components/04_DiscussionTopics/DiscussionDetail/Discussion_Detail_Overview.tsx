...
import { getLastSegment } from '../../01_LiveBoard/SH_Hazard/Select_image_section';
...
export default function DiscussionDetailOverView({
  data,
}: {
  ...
  return (
    <>
      ...
      <div className="flex gap-2">
        <div className="mx-2 my-4 flex w-full flex-col rounded-lg border-2 border-[#EEEEEE] p-4 shadow-sm shadow-primary-500 lg:mx-0 lg:ml-2">
        ...
        <div className="mx-2 my-4 flex w-full flex-col rounded-lg border-2 border-[#EEEEEE] p-4 shadow-sm shadow-primary-500 lg:mx-0 lg:ml-2">
          <div className="flex flex-col items-start justify-start gap-2 py-3">
            <span className="text-sm text-[#616161]">{'Assigned Project'}</span>
        ...
        <div className="mx-2 my-4 flex w-full flex-col rounded-lg border-2 border-[#EEEEEE] p-4 shadow-sm shadow-primary-500 lg:mx-0 lg:ml-2">
          <div className="flex flex-col justify-start gap-2 py-2">
            <span className="text-sm text-[#616161]">{'Submitted By'}</span>
          ...
          {data?.status?.toLowerCase() === 'close' && data?.closedBy && (
            <div className="flex flex-col justify-start gap-2 py-6">
              <span className="text-sm text-[#616161]">{'Closed By'}</span>

              <span className="flex cursor-pointer items-center gap-1">
                {data?.closedBy && (
                  <div className="flex items-center">
                    <PresignedUserAvatar
                      photo={data.closedBy.photo}
                      fallback="/images/User-profile.png"
                      alt="avatar"
                      className="mr-2 h-8 w-8 rounded-full border border-gray-500 text-[#616161]"
                    />
                    <span className="text-[#616161]">
                      {session?.user.user._id === data?.closedBy?._id ? (
                        'Me'
                      ) : (
                        <>
                          {data?.closedBy
      ...
      <div className="mx-2 my-4 flex flex-col rounded-lg border-2 border-[#EEEEEE] shadow lg:mx-0 lg:ml-2">
        ...
        <div className="flex flex-col flex-wrap items-start justify-start px-6">
          <div className="flex w-full justify-start">
            <div className="flex w-1/2 flex-col justify-start py-2">
              <span className="text-sm text-[#616161]">{'Topic Title'}</span>
              <span
                onClick={() => {
                  copyText({ text: data?.title ?? '' });
                }}
          ...
          {(data?.images ?? []).length > 0 && (
            <div className="flex w-full flex-wrap justify-start gap-3 pt-12">
              {(data?.images ?? []).map((val, index) => (
                <img
                  src={displayImages[index] ?? val}
                  alt={'image'}
                  key={index}
                  className="h-40 w-40 cursor-pointer rounded object-cover"
                  onClick={() => {
